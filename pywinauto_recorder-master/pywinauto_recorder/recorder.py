...
class IconSet:
	...
	hicon_clipboard = oaam.load_ico(path_icons + r'\Icons\paste.ico', 28, 28)
	hicon_light_on = oaam.load_ico(path_icons + r'\Icons\light-on.ico', 28, 28)
	hicon_record = oaam.load_ico(path_icons + r'\Icons\record.ico', 28, 28)
	hicon_power = oaam.load_ico(path_icons + r'\Icons\power.ico', 28, 28)
	hicon_stop = oaam.load_ico(path_icons + r'\Icons\stop.ico', 28, 28)
	hicon_search = oaam.load_ico(path_icons + r'\Icons\search.ico', 28, 28)
	hicon_play = oaam.load_ico(path_icons + r'\Icons\play.ico', 28, 28)
	hicon_pd = oaam.load_ico(path_icons + r'\Icons\p3.ico', 38, 38)
...
def write_in_file(events, out_filename, relative_coordinate_mode=False, menu_click=False):
	from pathlib import Path
	#home_dir = Path.home() / Path('Pywinauto recorder')
	#home_dir = Path('PywinAutoRecordedFiles')
	home_dir = Path('C:/PywinAutoRecordedFiles')
	home_dir.mkdir(parents=True, exist_ok=True)
	if out_filename:
		#record_file_name = home_dir / Path('recorded_' + out_filename + '.py')
		record_file_name = home_dir / Path(out_filename + '.py')
	else:
...
def overlay_add_progress_icon(main_overlay, i, x, y):
	...
	main_overlay.add(
		...
		color=(0, 0, 0), thickness=1, brush=oaam.Brush.solid, brush_color=color_)
	main_overlay.add(
	...
	for b in range(i % 6):
		...
		main_overlay.add(
			...
			color=(c, int(255 - c/2), c ), thickness=1, brush=oaam.Brush.solid, brush_color=color_)
	t0_progress_icon_timings = time.time()
...
def overlay_add_mode_icon(main_overlay, hicon, x, y):
	...
	'''
	main_overlay.add(
		geometry=oaam.Shape.rectangle, x=x, y=y, width=92, height=52,
		color=(0, 0, 0), thickness=1, brush=oaam.Brush.solid, brush_color=color_)
...
class Recorder(Thread):
		...
		for r_y in range(nb_y):
			for r_x in range(nb_x):
				...
				self.main_overlay.add(
					...
					height=r.height(),
					thickness=1, color=(255, 0, 0), brush=oaam.Brush.solid, brush_color=color_)
	...
	def __key_on(self, e):
		if (
			...
			if self.mode != "Record":
				...
				self.stop_recording(self.out_filename)
		elif (
	...
	def __display_info_tip(self, x, y, wrapper):
		...
		self.info_overlay.add(
			...
			thickness=1, color=(0, 0, 0), brush=oaam.Brush.solid, brush_color=color_)
		self.info_overlay.add(
			...
			font_size=16, text_color=(0, 0, 0), brush=oaam.Brush.solid, brush_color=color_,
		...
		if text3:
			...
			self.info_overlay.add(
				...
				height=tooltip3_height, thickness=1, color=(0, 0, 0), brush=oaam.Brush.solid, brush_color=color_)
			self.info_overlay.add(
	...
	def run(self):
		...
		print("COBOT RECORDER FOLDER:", dir_path)
				...
				if strategy == Strategy.unique_path:
					...
					if unique_candidate is not None:
						...
						self.main_overlay.add(
							...
							thickness=1, color=(0, 128, 0), brush=oaam.Brush.solid, brush_color=color_)
					else:
						for e in elements:
							r = e.rectangle()
							self.main_overlay.add(
	...
	def stop_recording(self,out_filename):
		if self.mode == "Record" and len(self.event_list) > 2:
			...
			clean_events(events)
			return write_in_file(events, out_filename, relative_coordinate_mode=self.relative_coordinate_mode)
		self.main_overlay.clear_all()
	...
	def quit(self):
		...
		self.mode = 'Quit'
		self.join()
		print('Cobot recording successful!')
		print("Quit")
