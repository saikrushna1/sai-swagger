...
class SysTrayIcon(object):
	...
	def __init__(self, icon, hover_text, menu_options, on_quit=None, default_menu_index=None, window_class_name=None, ):
		...
		menu_options = menu_options + [['Quit AND EXPORT', icon_power, self.QUIT], ]
...
def overlay_add_pywinauto_recorder_icon(overlay, x, y):
	overlay.add(
		geometry=oaam.Shape.rectangle, x=x, y=y, width=200, height=100, thickness=5, color=(255, 255,255),
	...
	overlay.add(
		...
		text=u'COBOT', font_size=44, font_name='Impact')
	overlay.add(
...
def display_splash_screen():
	...
	if __version__ == latest_version:
		print("Your Cobot recorder is up to date.")
	else:
		print("cobot recorder v" + latest_version + " is available!")
	...
	#if __version__ == latest_version:
	text_lines[6] = 'Cobot recorder v1.00' 
	#else:
	...
	text_lines[12] = 'Copy python code in clipboard                    				   Display / Hide information'
	text_lines[13] = 'CTRL+SHIFT+F                                                  				 CTRL+SHIFT+D'
	text_lines[15] = '                                                 					          Click on "Quit" in tray'
	text_lines[16] = '                                                  			                         menu to quit'
	...
	splash_background.add(
		...
		xyrgb_array=((x, y,  70,130,180), (x + w, y, 25,25,112), (x + w, y + h, 70,130,180)))
	splash_background.add(
		geometry=oaam.Shape.triangle, thickness=0,
		xyrgb_array=((x + w, y + h,  25,25,112), (x, y + h, 70,130,180), (x, y,  70,130,180)))
	...
	while continue_after_splash_screen:
		...
		overlay_add_mode_icon(splash_foreground, IconSet.hicon_clipboard,  splash_left + 50, splash_top + line_height * 11.7)
		overlay_add_mode_icon(splash_foreground, IconSet.hicon_power, int(splash_right - (52 + 50)), splash_top + line_height * 14.7)
		overlay_add_mode_icon(splash_foreground, IconSet.hicon_pd, splash_left + 50, int(splash_top + line_height * 19.3))
		
		
		splash_foreground.refresh()
		time.sleep(0.4)
		if n % 2 == 0:
			text_lines[19] = message_to_continue
		else:
			text_lines[19] = ''
...
if __name__ == '__main__':
	...
	parser.add_argument(
		"out_filename", help="Output file name after recording screen", type=str, default='')
	...
	if args.filename:
		...
		recorder = Recorder()
		# Start recording automatic when run python file
		...
		hover_text = "Cobot recorder"

		recorder.out_filename = args.out_filename
		def action_record(sysTrayIcon):
			if recorder.mode == 'Record':
				recorder.stop_recording(args.out_filename)
			else:
				recorder.start_recording()
		...
		def action_open_explorer(sysTrayIcon):
			pywinauto_recorder_path =  Path("Cobot recorder")
