...
//	}
	...
	public Status parseFile(Job job, String appName) throws IOException {
		Status status = new Status();
		String line;
		...
		Files.deleteIfExists(Paths.get(
				scriptsRootDir + appName + File.separator  + Constants.CONVERTED_SCRIPTS_PATH + File.separator + job.getScriptFileName() + ".sh"));
		...
		BufferedReader br = new BufferedReader(new FileReader(new File(
				scriptsRootDir+ appName + File.separator  + Constants.RECORDED_SCRIPTS_PATH + File.separator + job.getScriptFileName() + ".txt")));
		...
		String scriptoutputFile = scriptsRootDir+ appName + File.separator  + Constants.PLAYBACK_SCRIPTS_PATH + File.separator + job.getScriptFileName() + ".txt";
		...
		while ((line = br.readLine()) != null) {
			if (StringUtils.startsWith(line, "mysql> ")) {
				...
				psBuffer.append(pattern.split(line)[pattern.split(line).length - 1].trim() +  " " + scriptsRootDir + Constants.CONVERTED_SCRIPTS_PATH + "/" + job.getScriptFileName() + ".txt \n");
			} else {
...
//				+scriptsRootDir + appName + File.separator + Constants.PLAYBACK_SCRIPTS_PATH + File.separator + fileName + ".txt)2>&1");
	...
	public Status startVideo(String fileName, String format,String appName) throws Exception {
		...
		try {
			...
			String command = "/bin/bash "+ recordScript +" "+ scriptsRootDir + appName + File.separator + Constants.PLAYBACK_VIDEOS_PATH + File.separator + fileName + new Date().getTime() + ".mkv";
